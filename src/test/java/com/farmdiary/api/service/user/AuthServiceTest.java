...
class AuthServiceTest {
    ...
    void role_not_exists_then_thorw_ResourceNotFoundException() {
        ...
        when(roleRepository.findByName(GrantedRole.USER)).thenReturn(Optional.empty());
    ...
    void not_have_duplicated_information_then_signup_success() {
        ...
        Role role = Role.builder().name(GrantedRole.USER).build();
        Long roleId = 1L;
        ...
        when(roleRepository.findByName(GrantedRole.USER)).thenReturn(Optional.of(role));
    ...
    public void valid_login_info_then_login_success() {
        ...
        // when
        JwtResponse accessToken = authService.getAccessToken(loginRequest);
